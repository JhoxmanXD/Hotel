$(document).ready(function() {
    ...
    $('#example1').on('change', '.toggle-class', function() {
        ...
        var elementType = $(this).data('employee'); // Obtener el tipo de elemento (país, departamento, ciudad, etc.)
        var elementId = $(this).attr('employee-id'); // Obtener el ID del elemento
        var url; // Determinar la URL y los datos según el tipo de elemento

        // Configurar la URL y los datos según el tipo de elemento
        switch (elementType) {
            case 'employee':
                url = 'cambioestadoemployee';
                break;
